  ...
  function output_formtaxo_fields($type,$exc,$hide,$taxname,$taxlabel,$taxall,$opt,$c,$defaultclass,$formid,$divclass){
		...
		if($type == 'checkbox'){
 			if ( $count > 0 ){
				...
				$html .= '<label class="container"><input type="checkbox"  id="tchkb-'.$c.'-0" class="tchkb-'.$c.' chktaxoall" name="taxo['.$c.'][call]"  value="1" '.$checkall.'>'.$taxall.'<span class="checkmark"></span></label>';
				}
				$i = 1;
				foreach ( $terms as $term ) {
				$value = $term->slug;
				$checked = (isset($_GET['taxo'][$c]['term']) && in_array($value, $_GET['taxo'][$c]['term'])) ? 'checked="checked"' : '';
				$html .= '<label class="container"><input type="checkbox"  id="tchkb-'.$c.'-'.$i.'" class="tchkb-'.$c.'" name="taxo['.$c.'][term][]" value="'.$value.'" '.$checked.'>'.$term->name.'<span class="checkmark"></span></label>';
				$i++;
				}
				$html .= '</div>';
   ...
   function output_formcmf_fields($type,$metakey,$compare,$metaval,$label,$all,$i,$defaultclass,$id,$divclass ){
		 ...
		 if($type == 'checkbox'){
				...
				if(!empty($all)){
				 ...
				 $html .= '<label class="container"><input input type="checkbox"   id="cmf-'.$i.'-0" class="cmf-'.$i.' chkcmfall" name="cmf['.$i.'][call]"  value="1" '.$checkall.'>'.$all.'<span class="checkmark"></span></label>';
				}
				$c = 1;
				foreach ( $opts as $opt ) {
				        $val = explode('::',$opt);
					$checked = (isset($_GET['cmf'][$i]['value']) && in_array($val[0],$_GET['cmf'][$i]['value']) ) ? 'checked="checked"' : '';
				        $html .= '<label class="container"><input input type="checkbox"  id="cmf-'.$i.'-'.$c.'" class="cmf-'.$i.'" name="cmf['.$i.'][value][]" value="'.$val[0].'" '.$checked.'>'.$val[1].'<span class="checkmark"></span></label>';
						$c++;
					}
			 	$html .= '</div>';
